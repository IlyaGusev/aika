...
inline bool GzGetLine(gzFile file, std::string& line) {
  ...
  inline Move::Promotion PieceToPromotion(int p) {
    switch (p) {
      case -1:
  ...
  inline Move SanToMove(const std::string& san, const ChessBoard& board) {
    int p = 0;
    size_t idx = 0;
    if (san[0] == 'K') {
      p = 1;
...
public:
    ...
    void ParseLine(std::string& line, bool& started, bool& in_comment) {
        ...
        // Must have at least one non-tag non-empty line in order to be considered a game.
        started = true;
        ...
        while ((in_comment && line.find('}', cur_offset) != std::string::npos) ||
            ...
            if (in_comment && line.find('}', cur_offset) != std::string::npos) {
                line = line.substr(0, cur_offset) + line.substr(line.find('}', cur_offset) + 1);
                in_comment = false;
            } else {
